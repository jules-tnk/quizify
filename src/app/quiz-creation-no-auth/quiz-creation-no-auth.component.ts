...
@Component({
  selector: 'app-quiz-creation-no-auth',
  templateUrl: './quiz-creation-no-auth.component.html',
  styleUrls: ['./quiz-creation-no-auth.component.sass'],
})
export class QuizCreationNoAuthComponent {
  // @ts-ignore
  quizForm: FormGroup;
  ...
  ngOnInit(): void {
    this.categories.set('cat 1', 'one');
    this.categories.set('cat 2', 'two');
    this.categories.set('cat 3', 'three');

    this.lastValidCategory = this.categories.keys().next().value;

    this.initForm();

    this.filteredCategories = this.quizForm.controls[
      'categoryDisplay'
    ].valueChanges.pipe(
      startWith(''),
  ...
  fillFormWithQuizData(quiz: QuizModal): void {
    ...
    this.quizForm.setControl(
      'questions',
      this.fb.array(
        quiz.questions.map((q: QuestionModal) => this.initQuestionWithData(q))
      )
    );
  }
  ...
  updateLastValid(event: any, category: string) {
    if (event.isUserInput) {
      // @ts-ignore
      const categoryId = Array.from(this.categories.entries()).find(
  ...
  checkCategory() {
    if (
      !this.isOpened &&
      !this.isCategoryExists(this.quizForm.controls['categoryId'].value)
    ) {
      this.quizForm.controls['categoryId'].setValue(this.lastValidCategory);
      this.quizForm.controls['categoryDisplay'].setValue(
        this.categories.get(this.lastValidCategory)
      );
    }
  }
  ...
  addOption(questionIndex: number): void {
    const optionArray = (this.quizForm.get('questions') as FormArray)
      .at(questionIndex)
  ...
  mapQuestionsArrayToQuestionModalArray(
  ...
  mapFormToQuizModal(): QuizModal {
    const quizModal: QuizModal = {
      ...
      questions: this.mapQuestionsArrayToQuestionModalArray(
        this.quizForm.get('questions') as FormArray
      ),
